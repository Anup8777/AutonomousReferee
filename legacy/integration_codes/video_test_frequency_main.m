...
readyFlag =1; % Algorithm is ready to accept the current frame, and not busy 
cumulative_algorithm_time = 0;
...
current_frame_time = ii/FPS;

if ii > 0   
    if current_frame_time < cumulative_algorithm_time
        readyFlag =0;
    else
        readyFlag =1;
    end
...
if readyFlag ==1
    ...
    t_end = [t_end toc(tStart)];
    cumulative_algorithm_time = cumulative_algorithm_time + t_end(end);
...
fused_image = insertText(fused_image,[700 1000],'Checking','FontSize',33,'TextColor','white',...
    'BoxColor','green','BoxOpacity',0.1);

else
    fused_image = readFrame(v_in); 
        if ~isempty(AOI)
            fused_image = (imfuse((fused_image),AOI(:,:,1),'blend'));
            fused_image = (imfuse((fused_image),AOI(:,:,2),'blend'));
        end
end
